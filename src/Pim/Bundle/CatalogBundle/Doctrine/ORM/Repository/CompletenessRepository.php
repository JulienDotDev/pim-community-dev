...
SELECT t.label, COUNT(DISTINCT p.id) as total FROM pim_catalog_channel ch
    ...
    AND t.locale = '%current_user_locale%'
    GROUP BY ch.id, t.label
SQL;

        $sql = $this->applyTableNames($sql);
        $sql = $this->applyParameters($sql, $userLocale);
...
SQL;
     ...
     * @return string
     */
    protected function applyParameters($sql, $userLocale)
    {
        return strtr($sql, ['%current_user_locale%' => $userLocale]);
    }
}
